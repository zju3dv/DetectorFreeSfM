...
def read_megadepth_rgb(path, resize=None, df=None, padding=False, augment_fn=None):
    ...
    """
    # read image
    image_gray  = imread_gray(path, augment_fn, client=MEGADEPTH_CLIENT)
    ...
    image_gray = cv2.resize(image_gray, (w_new, h_new))
    scale = torch.tensor([w/w_new, h/h_new], dtype=torch.float)

    if padding:  # padding
        pad_to = max(h_new, w_new)
        image, _ = pad_bottom_right(image, pad_to, ret_mask=True)
        _, mask = pad_bottom_right(image_gray, pad_to, ret_mask=True)
    else:
        mask = None

    image = torch.from_numpy(image).float() / 255  # (3, h, w) -> (3, h, w) normalized
    mask = torch.from_numpy(mask)
    return image, mask, scale   
