...
class RandomConcatSampler(Sampler):
    ...
    NOTE: Don't re-initialize samplers between epochs (will lead to repeated sampling)
    ...
    def __iter__(self):
        ...
        for d_idx in range(self.n_subset):
            ...
            if self.subset_replacement:
                ...
                # pytorch doesn't have equivalence of `numpy.random.choice`
                ...
                else: # padding with torch.randint with replacement
        ...
        # shuffle the sampled dataset (from multiple subsets)
        if self.shuffle:
